...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward + 1 * self.discount * torch.logical_not(done) * next_qs
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        samples = action_distribution.rsample((1000,))
        
        # Calculate log probabilities of the samples
        log_probs = action_distribution.log_prob(samples)
        
        # Compute the entropy as the negative mean log probability
        entropy_values = -log_probs.mean(dim=0)
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            q_values = self.critic(obs.unsqueeze(0).repeat(self.num_actor_samples, 1, 1), action)
